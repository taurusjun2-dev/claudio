...
async function planDay() {
  ...
  const { systemPrompt } = await context.assemble('规划今天的音乐日程')
  const result = await llm.think(systemPrompt,
    '请为今天规划音乐日程，包含早晨、工作、下午、晚间风格建议，以及今天推荐歌单（5-10首）。say 字段留空。'
  )
...
async function morningBrief() {
  ...
  const { systemPrompt, userPrompt } = await context.assemble('早安播报')
  const result = await llm.think(systemPrompt,
...
async function hourlyMoodCheck() {
  ...
  const { systemPrompt, userPrompt } = await context.assemble('每小时情绪检查')
  const result = await llm.think(systemPrompt,
    userPrompt + '\n\n根据当前时间和天气，推荐1-2首合适的歌。say 字段留空，只填 play 和 reason。'
  )
